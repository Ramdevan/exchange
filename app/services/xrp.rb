...
class CoinRPC
  class XRP < BTC
    ...
    def handle_sign(data)
      ...
      result = JSON.parse( http_sign_post_request(post_body) )

      result.symbolize_keys! if result.is_a? Hash
      result
    end

    def http_sign_post_request(post_body)
    ...
    def getnewaddress
      ["#{get_account_address}?dt=#{generate_destination_tag}", get_account_passphrase]
    end
    ...
    def sign_transaction(from, to, amount)
      ...
      result = handle_sign params
      if result[:tx_blob].present?
    ...
    def get_account_passphrase
      @currency[:assets]['accounts'].first['passphrase']
    end
