class ApplicationController < ActionController::Base
  ...
  def redirect_back_or_settings_page
    if cookies[:redirect_to].present?
      ...
      redirect_to market_path(Market.default_one.id)
    end
  end
  ...
  def auth_admin!
    redirect_to settings_path unless is_admin?
  end
  ...
  def failed_two_factor_auth_key
    "citioption:session:#{request.ip}:failed_two_factor_auths"
  end
  ...
  def set_gon
    ...
    gon.fiat_currency = Currency.where(coin: false).first.code rescue ''#Currency.first.code
    ...
    if current_user
      ...
      gon.current_user = { sn: current_user.sn, total_balance: current_user.get_total_balance(conversion_unit), conversion_unit: conversion_unit.upcase }
  ...
  def save_session_key(member_id, key)
    Rails.cache.write "citioption:sessions:#{member_id}:#{key}", 1, expire_after: ENV['SESSION_EXPIRE'].to_i.minutes
  end

  def clear_all_sessions(member_id)
    if redis = Rails.cache.instance_variable_get(:@data)
      redis.keys("citioption:sessions:#{member_id}:*").each {|k| Rails.cache.delete k.split(':').last }
    end

    Rails.cache.delete_matched "citioption:sessions:#{member_id}:*"
  end
