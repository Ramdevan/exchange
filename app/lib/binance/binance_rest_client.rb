...
class BinanceRestClient
  ...
  def sync_orderbook
    Market.all.each do |market|
      ...
      Rails.cache.delete "xubiq:#{market}:depth:asks"
      Rails.cache.delete "xubiq:#{market}:depth:bids"
  ...
  def create_trade trade
    ...
    return if Rails.cache.read("xubiq:#{market.id}:push_binance_trade").present?
  ...
  def publish_trade(trade, market=nil)
    market ||= @market
    Rails.cache.write("xubiq:#{market.id}:push_binance_trade", true, expires_in: rand(1..5).seconds)
    AMQPQueue.publish(
      :trade,
      trade.as_json,
      { headers: {
