app.controller 'WithdrawsController', ['$scope', '$stateParams', '$http', '$gon', 'fundSourceService', 'ngDialog', ($scope, $stateParams, $http, $gon, fundSourceService, ngDialog) ->
  ...
  if defaultFundSource
    ...
    _selectedFundSourceTag = fund_sources[0].destination_tag if fund_sources.length
  ...
  @createWithdraw = (currency) ->
    ...
    data = { withdraw: { member_id: current_user.id, currency: currency, sum: @withdraw.sum, fund_source: _selectedFundSourceId, fund_tag: _selectedFundSourceTag } }
  ...
  $scope.openFundSourceManagerPanel = ->
    if $scope.currency == $gon.fiat_currency
      ...
      template = '/templates/fund_sources/coin_xrp.html'
      className = 'ngdialog-theme-default custom-width coin'
    else if $scope.currency == 'xmr'
      template = '/templates/fund_sources/coin_xmr.html'
      className = 'ngdialog-theme-default custom-width coin'
    else
