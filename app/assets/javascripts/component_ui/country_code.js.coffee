$ ->
  ...
  $('#sms_auth_country').on 'change', ->
    country_code = @value
    i = 0
    while i < allTimeZone.length
      if allTimeZone[i].Code == country_code
        document.getElementById('country_code').innerHTML = allTimeZone[i].MobileCode
      i++
    return
  return