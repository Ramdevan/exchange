@MarketData = flight.component ->
  ...
  @createPoint = (i, trade) ->
    ...
    while trade.date >= @next_ts
      ...
      @points.volume.push {x: x, y: v, color: if p >= @points.close[i][1] then 'rgba(0, 255, 0, 0.5)' else '#fa0069'}
      i += 1
    i
  ...
  @updatePoint = (i, trade) ->
    ...
    @points.volume[i].y += v
    @points.volume[i].color = if i > 0 && @points.close[i][1] >= @points.close[i-1][1] then 'rgba(0, 255, 0, 0.5)' else '#fa0069'
  ...
  @prepare = (k) ->
    ...
    for cur, i in k
      ...
      volume.push {x: time, y: vol, color: if trend then 'rgba(0, 255, 0, 0.5)' else '#fa0069'}
