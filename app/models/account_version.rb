class AccountVersion < ActiveRecord::Base
  ...
  HISTORY = [Account::STRIKE_ADD, Account::STRIKE_SUB, Account::STRIKE_FEE, Account::DEPOSIT, Account::WITHDRAW, Account::FIX, Account::REFERRAL_FEES]
  ...
  REASON_CODES = {
    ...
    Account::CROSS_STRIKE_ADD => 140,
    Account::CROSS_STRIKE_SUB => 150,
    ...
    Account::REFERRAL_FEES => 3000,
    Account::TRADE_FEES => 4000 }
  ...
  scope :admins, -> { where(member_id: Member.find_by_email(ENV['ADMIN']).id) }
  scope :created_between, lambda {|start_date, end_date| where(created_at: Date.parse(start_date).beginning_of_day..Date.parse(end_date).end_of_day )}
