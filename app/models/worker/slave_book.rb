  ...
  class SlaveBook
    ...
    def process(payload, metadata, delivery_info)
      ...
      when 'add'
        book.add order
      when 'update'
        book_order = book.find(order)
        book_order.volume = order.volume if book_order # only volume would change
      when 'remove'
        book.remove order
      else
    ...
    def cache_book
      @managers.keys.each do |id|
        ...
        Rails.cache.write "axios:#{market}:depth:asks", get_depth(market, :ask)
        Rails.cache.write "axios:#{market}:depth:bids", get_depth(market, :bid)
