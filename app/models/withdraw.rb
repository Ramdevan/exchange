class Withdraw < ActiveRecord::Base
  ...
  after_create :send_email, :send_sms
  ...
  def min_withdraw
    errors.add(:amount, :minimum, amount: channel.min_withdraw) if sum < channel.min_withdraw
  end
  ...
  aasm :whiny_transitions => false do
    ...
    state :submitted,   after_commit: [:send_email, :send_sms]
    ...
    state :almost_done
    state :done,        after_commit: [:send_email, :send_sms, :send_fee]
  ...
  def send_sms
    ...
    case aasm_state
    when 'submitting'
      sms_message = I18n.t('sms.withdraw_submit', name: (member.name || member.email),
                           currency: currency_text,
                           amount: amount)
    when 'done'
      sms_message = I18n.t('sms.withdraw_done', name: (member.name || member.email),
                           currency: currency_text,
                           amount: amount)
    end
  ...
  def send_fee
    ...
    admin.accounts.find_by_currency(currency).lock!.plus_funds(fee, reason: Account::WITHDRAW, ref: self)
  end

  def send_coins!
    Rails.logger.info "WITHDRAW STARTED send_coins! *** -- #{coin?}"
