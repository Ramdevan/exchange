class Deposit < ActiveRecord::Base
  ...
  # AS INITIAL STATE IN AASM HAS ISSUE SENDING MAIL/SMS, MOVING IT TO AFTER_CREATE
  ...
  aasm :whiny_transitions => false do
    ...
    state :submitted
    state :rejected
    state :accepted, after_commit: [:do, :send_email, :send_sms]
    state :checked
    state :warning
  ...
  def display_type
    # self.class.name.split('::').first.singularize rescue 'Deposit'
    'Deposit'
  end
  ...
  def send_sms
    ...
    case aasm_state
    when 'submitting'
      sms_message = I18n.t('sms.deposit_submit', name: (member.name || member.email),
                           currency: currency_text,
                           amount: amount)
    when 'accepted'
      sms_message = I18n.t('sms.deposit_done', name: (member.name || member.email),
                           currency: currency_text,
                           amount: amount)
    end
